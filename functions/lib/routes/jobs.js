...
router.get('/:id', async (req, res) => {
    try {
        ...
        const doc = await firebase_1.db.collection('jobs').doc(id).get();
        if (!doc.exists) {
...
router.post('/upload', (req, res) => {
    ...
    uploadHandler(req, res, async (err) => {
        try {
            ...
            const gcsFile = firebase_1.bucket.file(path);
            ...
            const publicUrl = `https://firebasestorage.googleapis.com/v0/b/${firebase_1.bucket.name}/o/${encodeURIComponent(path)}?alt=media&token=${token}`;
...
router.post('/', async (req, res) => {
    try {
        ...
        const ref = await firebase_1.db.collection('jobs').add(payload);
